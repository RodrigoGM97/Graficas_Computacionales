...
function createSquare(gl) 
{
    ...
    //Draws the vertices of the figure
    var verts = [
        -0.5,  0.0,  0.0,
        0.25,  0,  0.0,
        -0.5, -0.75,  0.0,
    ...
    //Set number of vertices and drawing format    
...
function createSphere(gl, radius)
{
    ...
    var verts = [0,0,0];
    //Since a pacman is not a full circle I decided to start at 40°
    var angle = 40;
    //Cicle through the angles until the pacman is built
    while (angle<=320)
    {
        verts.push(radius * Math.cos(angle*0.0174533)); //For x cords get the cos of the angle 0.0.17 is the conversion to radian
        verts.push(radius * Math.sin(angle*0.0174533)); //For y cords get the sin of the angle 0.0.17 is the conversion to radian
        verts.push(0); //Z coordinate wont change so just keep it in the same plane
        angle += 1; //Iterate through the angles
    }
    gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(verts), gl.STATIC_DRAW);
    console.log("Verts: "+verts);
    console.log("Nverts: "+verts.length);
    //Set the number of vertices with how many dots where filled with the iteration
    //Divided by 3 because of the 3 planes x,y,z
